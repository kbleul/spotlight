    ...
    >
      <section className="flex items-start justify-between">
        ...
        >
          ...
          <p className="ml-[25%] mt-4 pr-[7%] text-[#B9B9B9]">
...
const ProjectsGrid = () => {
  ...
  return (
    <article ref={ref} className="mt-10 zw-full flex justify-center">
      <motion.div
        className="w-4/5 gap-6 flex justify-center items-center"
        initial={inView ? { y: -1000, x: 0 } : { y: 0, x: 0 }}
        animate={inView ? { y: 0, x: 0 } : { y: 1000, x: 0 }}
        transition={{ duration: 0.8 }}
      >
        <section className="w-1/3 flex flex-col gap-4 rounded-md">
          <ProjectCard />
          <ProjectCard />
        </section>
        <section className="w-1/3  flex flex-col gap-4 mt-[15vh]">
          <ProjectCard />
          <section className="h-[50vh] px-4 bg-white text-black rounded-md flex flex-col gap-4 justify-center items-center">
            <h4 className="font-extrabold w-full text-center">
              Are You Looking For Something Special Or Diffrent?
            </h4>
